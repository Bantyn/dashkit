    ...
    <div class="flex-1 overflow-y-auto bg-gray-50 min-h-screen">
      <main class="p-6 xl:p-10 max-w-full mx-auto">
        <div class="mb-8">
          ...
          <p class="text-gray-500 mt-1">Configure global platform details and support channels.</p>
        </div>
        ...
        <form (submit)="saveSettings($event)" class="space-y-8">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <app-ui-input
              ...
              label="Support Email"
              type="email"
              placeholder="support@clothify.com"
            ></app-ui-input>
          </div>
...
export class SettingsGeneralComponent implements OnInit {
  platformName = 'Clothify SaaS';
  supportEmail = 'support@clothify.com';
  ...
  ngOnInit() {
    this.loadTelemetrySettings();
  }

  loadTelemetrySettings() {
    this.loading = true;
    this.api.getPlatformTelemetrySettings().subscribe({
      next: (res) => {
        if (res.success && res.data) {
          this.telemetryEnabled = res.data.enabled;
        }
        this.loading = false;
      },
      error: () => {
        this.toast.showError('Failed to load telemetry settings');
        this.loading = false;
      }
    });
  }
  ...
  saveSettings(e: Event) {
    ...
    this.api.updatePlatformTelemetrySettings({ enabled: this.telemetryEnabled }).subscribe({
      next: () => {
        this.saving = false;
