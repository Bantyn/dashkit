...
@Component({
  ...
  template: `
    <div class="h-full bg-[#f5f7fa] flex overflow-hidden">
      ...
      <div
        class="w-[40%] min-w-[350px] max-w-[500px] bg-white border-l border-gray-200 flex flex-col h-full sticky top-0 no-print"
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          >
            ...
            <div>
              ...
              <div class="text-sm text-gray-500 mt-1 flex justify-center items-center gap-2">
                <i class="bi bi-telephone-fill"></i> {{ selectedCustomer.phoneNumber || 'N/A' }}
              </div>
            </div>
          ...
          <!-- CRM Info (DOB, Anniversary, Segment) -->
          @if (selectedCustomer.dateOfBirth || selectedCustomer.anniversaryDate || selectedCustomer.segment) {
            <div class="mb-6 bg-indigo-50 border border-indigo-100 rounded-xl p-4 space-y-2">
              <div class="text-xs font-bold text-indigo-700 uppercase tracking-wider mb-3 flex items-center gap-2">
                <i class="bi bi-people-fill"></i> CRM Details
              </div>
              @if (selectedCustomer.segment) {
                <div class="flex justify-between items-center">
                  <span class="text-xs text-gray-500">Segment</span>
                  <span class="text-xs px-2 py-0.5 rounded-full font-bold capitalize bg-purple-100 text-purple-700">{{ selectedCustomer.segment }}</span>
                </div>
              }
              @if (selectedCustomer.dateOfBirth) {
                <div class="flex justify-between items-center">
                  <span class="text-xs text-gray-500"><i class="bi bi-cake2-fill text-pink-400 mr-1"></i>Date of Birth</span>
                  <span class="text-xs font-semibold text-gray-700">{{ selectedCustomer.dateOfBirth | date:'d MMM yyyy' }}</span>
                </div>
              }
              @if (selectedCustomer.anniversaryDate) {
                <div class="flex justify-between items-center">
                  <span class="text-xs text-gray-500"><i class="bi bi-heart-fill text-rose-400 mr-1"></i>Anniversary</span>
                  <span class="text-xs font-semibold text-gray-700">{{ selectedCustomer.anniversaryDate | date:'d MMM yyyy' }}</span>
                </div>
              }
            </div>
          }
      ...
      >
        ...
        <p class="text-gray-500 text-sm">
          Select a customer from the list to view their purchase history.
        </p>
      </div>
...
export class CustomerListComponent implements OnInit {
  ...
  get filteredCustomers(): Customer[] {
    ...
    return this.customers.filter(
      (customer) =>
        customer.name.toLowerCase().includes(query) || customer.phoneNumber.includes(query),
    );
  }
