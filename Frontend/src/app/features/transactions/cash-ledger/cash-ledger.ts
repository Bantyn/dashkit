    ...
    <div class="h-full bg-[#f5f7fa] flex overflow-hidden">
      ...
      <div class="flex-1 flex flex-col min-w-0 border-r border-gray-200 bg-gray-50">
        ...
        <div class="px-6 py-5 border-b border-gray-200 bg-white flex justify-between items-center shrink-0">
          <div>
            <h2 class="text-xl font-bold text-gray-900">Cash Transactions</h2>
            <p class="text-xs text-gray-400 mt-0.5">COD orders — in-store & website cash on delivery</p>
          </div>
        ...
        <div class="px-6 py-4 bg-white border-b border-gray-100 shrink-0 grid grid-cols-3 gap-3">
          <div class="bg-green-50 border border-green-100 rounded-xl p-3 text-center">
            <div class="text-xs text-green-600 font-semibold uppercase tracking-wider mb-1">Total Cash</div>
            <div class="text-lg font-black text-green-700">₹{{ totalRevenue | number }}</div>
          </div>
          <div class="bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-center">
            <div class="text-xs text-emerald-600 font-semibold uppercase tracking-wider mb-1">Collected</div>
            <div class="text-lg font-black text-emerald-700">₹{{ paidRevenue | number }}</div>
          </div>
        ...
        <div class="px-4 py-3 bg-white shrink-0 border-b border-gray-100 flex items-center gap-3">
          <div class="relative flex-1">
            ...
            <input
              type="text"
              [(ngModel)]="searchQuery"
              placeholder="Search customer name or status..."
              class="w-full pl-9 pr-4 py-2 bg-gray-50 border border-transparent focus:bg-white focus:border-green-400 focus:ring-2 focus:ring-green-100 rounded-lg text-sm transition-all"
            />
          </div>
              ...
              <tbody class="divide-y divide-gray-100">
                @if (loading) {
                  <tr>
                    <td colspan="5" class="px-6 py-10 text-center">
                      <div class="flex flex-col items-center gap-3">
                        ...
                        <span class="text-sm text-gray-400">Loading cash transactions...</span>
                      </div>
                    </td>
                  </tr>
                } @else if (error) {
                    ...
                    <td colspan="5" class="px-6 py-10 text-center">
                      <div class="flex flex-col items-center gap-4">
                        <div class="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center">
                          <i class="bi bi-cash-stack text-green-300 text-xl"></i>
                        </div>
                        <span class="text-gray-400 text-sm">
                          {{ transactions.length === 0 ? 'No cash transactions found.' : 'No records match your search.' }}
                        </span>
                      </div>
                    </td>
                  </tr>
                  ...
                  @for (txn of filteredEntries; track txn.id) {
                    <tr
                      (click)="select(txn)"
                      class="cursor-pointer transition-colors hover:bg-green-50/40 group"
                      [class.bg-green-50]="selected?.id === txn.id"
                    >
                      <td class="px-6 py-4">
      ...
      @if (selected) {
        <div class="w-[40%] min-w-[340px] max-w-[480px] bg-white border-l border-gray-200 flex flex-col overflow-hidden">
          <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-start shrink-0 bg-gray-50">
            <div>
              <h3 class="text-lg font-bold text-gray-900">Cash Transaction</h3>
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div class="bg-white border border-gray-100 rounded-xl shadow-sm p-5 text-center">
              <div class="w-14 h-14 rounded-full bg-green-100 text-green-700 flex items-center justify-center font-black text-2xl mx-auto mb-3">
              ...
              <div class="flex border-t border-gray-100 mt-4 pt-4 divide-x divide-gray-100">
                <div class="flex-1">
                  ...
                  <div class="text-xl font-black text-green-700">₹{{ selected.amount | number }}</div>
                </div>
                <div class="flex-1">
            ...
            <div class="bg-white border border-gray-100 rounded-xl shadow-sm">
              ...
              <div class="divide-y divide-gray-50">
                <div class="px-4 py-3 flex justify-between">
                  ...
                  <span class="text-sm font-semibold text-gray-900">{{ selected.method }}</span>
                </div>
                <div class="px-4 py-3 flex justify-between">
            ...
            <!-- Total Amount Block -->
            <div class="bg-green-800 rounded-xl p-5 text-white flex justify-between items-center">
              <div>
                <div class="text-xs text-green-300 uppercase tracking-wider mb-1">Cash Amount</div>
                <div class="text-2xl font-black">₹{{ selected.amount | number }}</div>
              </div>
              <div class="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center">
                <i class="bi bi-cash text-lg text-white"></i>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hidden md:flex flex-col items-center justify-center w-[40%] min-w-[340px] max-w-[480px] bg-white border-l border-gray-200 text-center p-8">
          <div class="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mb-4">
            <i class="bi bi-cash-stack text-2xl text-green-300"></i>
          </div>
          <h3 class="text-gray-900 font-medium mb-1">No transaction selected</h3>
          <p class="text-gray-400 text-sm">Click any row to view cash transaction details.</p>
        </div>
      }
    </div>
  `,
})
export class CashLedger implements OnInit {
  ...
  load() {
    ...
    this.txnService.getCashTransactions(this.shopId).subscribe({
      next: (res) => {
      ...
      error: () => {
        this.error = 'Failed to load cash transactions.';
        this.loading = false;
        this.cdr.detectChanges();
      },
    });
