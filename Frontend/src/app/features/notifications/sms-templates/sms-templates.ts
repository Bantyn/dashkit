...
export class SmsTemplates implements OnInit {
  ...
  loadTemplates() {
    ...
    this.notificationService.getTemplates(this.shopId).subscribe({
      next: (res) => {
        // filter for SMS templates
        this.templates = (res.data || []).filter((t) => t.type === 'sms');
  ...
  sendTest() {
    if (!this.testRecipient) {
      this.toastService.showWarning('Please enter a test phone number.');
      return;
    }
    ...
    this.notificationService.sendTestNotification(payload).subscribe({
      next: () => {
        this.toastService.showSuccess('Test SMS dispatched! Check details in Notification Logs.');
        this.isSendingTest = false;
      },
      error: (err) => {
        this.toastService.showError(`Failed to send test SMS: ${err.error?.error?.message || err.message}`);
        this.isSendingTest = false;
      },
    });
  }
