...
@Component({
  ...
  styleUrl: './whatsapp-templates.css',
})
export class WhatsappTemplates implements OnInit {
  ...
  loadTemplates() {
    ...
    this.notificationService.getTemplates(this.shopId).subscribe({
      next: (res) => {
        ...
        this.templates = (res.data || []).filter((t) => t.type === 'whatsapp');
  ...
  sendTest() {
    ...
    this.notificationService.sendTestNotification(payload).subscribe({
      next: () => {
        this.toastService.showSuccess('Test WhatsApp dispatched! Check details in Notification Logs.');
        this.isSendingTest = false;
      },
      error: (err) => {
        this.toastService.showError(`Failed to send test WhatsApp: ${err.error?.error?.message || err.message}`);
        this.isSendingTest = false;
      },
    });
  }
