...
export class WebsiteLayoutComponent implements OnInit, OnDestroy {
  ...
  get announcementTexts(): string[] {
    ...
    return this.offers.map((offer) => {
      switch (offer.type) {
        ...
        case 'flat':
          return offer.code
            ? `GET FLAT ₹${offer.value} OFF! Use code: ${offer.code}`
            : `GET FLAT ₹${offer.value} OFF ON ALL orders!`;
        case 'buy_x_get_y':
          return `BUY ${offer.buyX} GET ${offer.getY} FREE!`;
        case 'free_shipping':
          return `FREE SHIPPING on orders of ₹${offer.minCalculatedAmount || 0} or more!`;
        default:
          return offer.title;
      }
    });
  ...
  ngOnInit() {
    ...
    this.shopConfig$.subscribe((config) => {
      if (config) {
        ...
        this.websiteService.getOffers().subscribe({
          next: (offers) => {
            this.offers = (offers || []).filter(o => o.isActive && o.showOnWebsite !== false);
          },
          error: (err) => {
