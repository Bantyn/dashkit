    ...
    <div class="bg-white min-h-screen pt-4 pb-24 text-gray-900">
      <div class="container mx-auto px-4 max-w-7xl animate-fade-in-up">
        <ng-container *ngIf="product$ | async as product; else loading">
          ...
          >
            <a [routerLink]="routePrefix" class="hover:text-black transition-colors">Home</a>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 xl:gap-16">
            ...
            <div class="lg:col-span-7">
              ...
              <div class="hidden lg:flex flex-col gap-6 w-full">
                <div
                  *ngFor="let img of getStackedImages(product)"
                  class="w-full aspect-[3/4] bg-[#f8f9ff] overflow-hidden rounded-2xl border border-gray-50 shadow-sm relative group"
                >
                  <img
              ...
              <div
                class="lg:hidden relative w-full aspect-[3/4] bg-[#f8f9ff] overflow-hidden mb-6 rounded-2xl"
              >
                <div
                ...
                <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                  <span
                    *ngFor="let img of getStackedImages(product); let idx = index"
                    class="w-1.5 h-1.5 rounded-full transition-all duration-300"
                    [class.bg-black]="currentMobileSlide === idx"
                    [class.bg-black/20]="currentMobileSlide !== idx"
                  ></span>
                </div>
              </div>
            </div>
              ...
              <div class="lg:sticky lg:top-24 h-fit text-left">
                  ...
                  @if (
                    ...
                    <div class="flex items-baseline gap-2">
                      <span class="text-2xl font-bold text-gray-900">
                        {{
                          (selectedVariant$ | async)?.wholesalePrice ||
                            product.variants[0]?.wholesalePrice | currency: 'INR'
                        }}
                      </span>
                ...
                <div class="space-y-6 mb-8">
                  ...
                  <div *ngIf="hasColors(product)">
                    ...
                    >
                      Product Color:
                      <span class="text-gray-900 capitalize">{{ selectedColor }}</span>
                    </label>
                    <div class="flex flex-wrap gap-2.5">
                      <button
                        *ngFor="let variant of getUniqueColors(product)"
                        (click)="
                          isColorAvailable(product, variant.color!) && selectColor(variant.color!)
                        "
                        ...
                        [class.border-gray-200]="selectedColor !== variant.color"
                        [class.opacity-40]="!isColorAvailable(product, variant.color!)"
                        [class.cursor-not-allowed]="!isColorAvailable(product, variant.color!)"
                      ...
                      >
                        ...
                        >
                          <div class="w-[120%] h-[1px] bg-red-500 rotate-45 z-10"></div>
                        </div>
                      </button>
                    </div>
                  </div>
                  ...
                  <div *ngIf="hasSizes(product)">
                    ...
                    >
                      <span
                        >Product Size:
                        <span class="text-gray-900 uppercase font-extrabold">{{
                          selectedSize || 'Select'
                        }}</span></span
                      >
                      <button
                        class="underline text-gray-900 hover:text-gray-600 focus:outline-none text-xs"
                      >
                        Size Guide
                      </button>
                    </div>
                    <div class="grid grid-cols-5 gap-2">
                      <button
                        *ngFor="let variant of getAvailableSizes(product)"
                        (click)="
                          isSizeAvailable(product, variant.size!) && selectSize(variant.size!)
                        "
                        ...
                        [class.bg-black]="selectedSize === variant.size"
                        [class.text-white]="selectedSize === variant.size"
                        [class.border-gray-200]="selectedSize !== variant.size"
                        [class.text-gray-900]="selectedSize !== variant.size"
                        [class.hover:border-black]="
                          selectedSize !== variant.size && isSizeAvailable(product, variant.size!)
                        "
                        [disabled]="!isSizeAvailable(product, variant.size!)"
                        [class.opacity-40]="!isSizeAvailable(product, variant.size!)"
                        [class.cursor-not-allowed]="!isSizeAvailable(product, variant.size!)"
                      >
                        {{ variant.size }}
                        <div
                          *ngIf="!isSizeAvailable(product, variant.size!)"
                          class="absolute inset-0 flex items-center justify-center pointer-events-none"
                        >
                          <div class="w-[120%] h-[1px] bg-red-500 rotate-12 z-10 opacity-70"></div>
                        </div>
                      </button>
                    </div>
                  </div>
                  ...
                  <div class="flex gap-4">
                    ...
                    >
                      <button
                        ...
                        class="w-9 h-full flex items-center justify-center text-gray-500 hover:text-black hover:bg-gray-50 transition-colors focus:outline-none"
                      >
                        <i class="bi bi-dash"></i>
                      </button>
                      <span class="flex-1 text-center font-bold text-sm text-gray-900 select-none">
                        {{ quantity }}
                      </span>
                      <button
                    ...
                    <button
                      ...
                      [disabled]="!isValidSelection(product) || isAdding || isOutOfStock(product)"
                      class="flex-[2] bg-black text-white h-12 rounded-lg font-bold hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex justify-center items-center gap-2 text-xs uppercase tracking-wider"
                    >
                    ...
                    <button
                      ...
                      class="flex-1 border border-gray-200 bg-white text-gray-900 h-12 rounded-lg font-bold hover:border-gray-300 transition-all flex justify-center items-center gap-2 text-xs uppercase tracking-wider"
                    >
                      <i
                        class="bi text-lg"
                ...
                >
                  <p class="font-bold text-amber-900 text-xs uppercase tracking-wider mb-1">
                    Showcase Item
                  </p>
                  <p class="text-xs text-amber-700 leading-relaxed">
                    This product is for display only. To purchase or enquire, please contact the
                    shop directly.
                  </p>
                </div>
                ...
                <div class="border-t border-gray-100 pt-2">
                  ...
                  <div class="border-b border-gray-100 py-4">
                    <button
                      ...
                      class="w-full flex justify-between items-center text-xs font-bold uppercase tracking-wider text-gray-900 focus:outline-none"
                    >
                    ...
                    >
                      This item is available in limited quantities at select store locations. Please
                      contact your nearest store branch for stock reservation.
                    </div>
                  </div>
                  ...
                  <div class="border-b border-gray-100 py-4">
                    ...
                    <div
                      *ngIf="accordions.fit"
                      class="mt-3 text-xs text-gray-500 leading-relaxed animate-fade-in"
                    >
                      Designed for a relaxed fit, matching standard contemporary sizing. Model is
                  ...
                  <div class="border-b border-gray-100 py-4">
                    ...
                    <div
                      *ngIf="accordions.fabric"
                      class="mt-3 text-xs text-gray-500 leading-relaxed animate-fade-in"
                    >
                      Made with a premium blend of 70% baby alpaca and 30% extrafine virgin wool.
                  ...
                  <div class="border-b border-gray-100 py-4">
                    ...
                    <div
                      *ngIf="accordions.shipping"
                      class="mt-3 text-xs text-gray-500 leading-relaxed animate-fade-in"
                    >
                      Complimentary standard shipping on orders over INR 8,000. Returns are accepted
                      within 14 days of purchase in original unworn condition.
                    </div>
                  </div>
                </div>
                ...
                >
                  <div class="flex gap-2">
                    ...
                    <span class="text-gray-900">{{
                      (selectedVariant$ | async)?.sku || product.variants[0]?.sku
                    }}</span>
                  </div>
                  <div class="flex gap-2" *ngIf="product.tags.length">
                    <span class="w-16">Tags:</span>
                    <span class="text-gray-900">{{ product.tags.join(', ') }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            <div>
              <span class="text-[9px] font-bold text-gray-400 tracking-widest uppercase block mb-3"
                >Design</span
              >
              <h4 class="text-sm font-bold text-gray-900 mb-2">Airy & Warm</h4>
              <p class="text-xs text-gray-500 leading-relaxed">
                This piece features a bold cable knit design, structured mock neck, and functional
                side slits, perfect for versatile layering with your wardrobe staples.
              </p>
            </div>
            <div>
              <span class="text-[9px] font-bold text-gray-400 tracking-widest uppercase block mb-3"
                >Quality</span
              >
              <h4 class="text-sm font-bold text-gray-900 mb-2">Made in Italy</h4>
              <p class="text-xs text-gray-500 leading-relaxed">
                Fashioned by an Italian mill dedicated to renewable fibers, following sustainable
                environmental and social standards for a premium finish.
              </p>
            </div>
            <div>
              <span class="text-[9px] font-bold text-gray-400 tracking-widest uppercase block mb-3"
                >Sustainability</span
              >
              <h4 class="text-sm font-bold text-gray-900 mb-2">Sustainable Baby Alpaca</h4>
              <p class="text-xs text-gray-500 leading-relaxed">
                Made using highest quality Baby Alpaca from Peru (certified mulesing free) and
                blended with recycled polyamide for durability.
              </p>
            </div>
          </div>

          <!-- Customer Reviews Section -->
          <div class="max-w-5xl mx-auto px-4 my-20 border-t border-gray-100 pt-20">
            <h3 class="text-2xl font-black text-gray-900 tracking-tight text-left mb-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <div class="lg:col-span-4 text-left space-y-6">
                <div *ngIf="reviews$ | async as reviews" class="space-y-2">
                  <app-review-summary-card
                    ...
                    [summaryText]="'Based on ' + reviews.length + ' customer reviews'"
                  ></app-review-summary-card>
                </div>

                <button
                  (click)="showReviewForm = !showReviewForm"
                  class="w-full py-3.5 text-xs font-bold uppercase tracking-widest border border-black hover:bg-black hover:text-white transition-all rounded-md focus:outline-none"
                >
                ...
                >
                  ...
                  <form (ngSubmit)="submitReview()" #reviewForm="ngForm" class="space-y-4">
                    <div>
                      <label class="block text-[9px] font-bold uppercase text-gray-400 mb-1"
                        >Your Name</label
                      >
                      <input
                        type="text"
                    ...
                    <button
                      ...
                      class="w-full py-3 text-xs font-bold uppercase tracking-wider bg-black text-white rounded-lg hover:opacity-90 disabled:opacity-40 transition-opacity"
                    >
                      Submit Review
                    </button>
                  </form>
              ...
              <div class="lg:col-span-8 text-left">
                <ng-container *ngIf="reviews$ | async as reviews">
                  ...
                  >
                    <i class="bi bi-chat-square-quote text-3xl text-gray-300 mb-2 inline-block"></i>
                    <p class="text-sm font-semibold text-gray-900">Be the first to review</p>
                  ...
                  <div class="divide-y divide-gray-100">
                    ...
                    >
                      <div class="flex justify-between items-start mb-2">
                        <div>
                          <h5 class="text-sm font-bold text-gray-900">{{ review.customerName }}</h5>
                          <span
                            class="inline-flex items-center gap-1.5 text-[9px] font-bold uppercase text-gray-400 tracking-wider mt-0.5"
                          >
                            <i class="bi bi-patch-check-fill text-green-600 text-xs"></i> Verified
                            Buyer
                          </span>
                        </div>
                  ...
                  >
                    ...
                    <button
                      ...
                      "
                      [disabled]="
                        reviewsCurrentPage === Math.ceil(reviews.length / reviewsPageSize)
                      "
                      class="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-black disabled:opacity-40 disabled:hover:border-gray-200 transition-colors focus:outline-none"
                    >
          ...
          >
            <h3 class="text-xl font-black text-gray-900 mb-8 tracking-tight">Style With</h3>
              ...
              >
                ...
                >
                  ...
                  <div class="relative aspect-[3/4] bg-[#f8f9ff] overflow-hidden mb-4 rounded-xl">
                    <img
                      [src]="
                        ((product.images && product.images[0]) | optimizeImage: 'card') || '/Cloth_placeholder.png'
                      "
                    ...
                    >
                      ...
                      <span
                        *ngIf="isLoadingMap[product.id]"
                        class="w-4.5 h-4.5 border-2 border-gray-300 border-t-gray-900 rounded-full animate-spin"
                      ></span>
                    </button>
                    ...
                    >
                      <span class="text-[9px] font-bold text-gray-900 uppercase tracking-widest"
                        >Colors</span
                      >
                  ...
                  <h4
                    class="text-xs font-semibold text-gray-900 leading-tight group-hover:underline"
                  >
                    {{ product.name }}
                  </h4>
          ...
          >
            <div *ngIf="recently.length > 0">
              <h3 class="text-xl font-black text-gray-900 mb-8 tracking-tight">Recently viewed</h3>
              <div class="grid grid-cols-2 md:grid-cols-3 gap-8">
                <div
...
export class WebsiteProductDetailComponent implements OnInit {
  ...
  constructor(
    ...
    this.product$ = this.route.paramMap.pipe(
      switchMap((params) => {
        ...
        this.loadReviews(id); // Load reviews when product changes
      ...
      tap((product) => {
        ...
        if (product.variants?.length) {
          ...
          // Initialize selectors if applicable
          if (product.variants[0].color) this.selectedColor = product.variants[0].color;
          if (product.variants[0].size) this.selectedSize = product.variants[0].size;

          // Eagerly preload variant specific images in the background so switching colors is instant
          setTimeout(() => {
            product.variants.forEach((variant) => {
              if (variant.images?.length) {
                variant.images.forEach((img) => {
                  ...
                  thumbPreloader.src = this.optimizeImagePipe.transform(img, 'card');
                });
              }
            });
          }, 100);
      ...
      switchMap((product) => {
        return this.websiteService.getProducts({}).pipe(
          map((allProducts) => {
            ...
            // 1. Get products in the same collection
            let suggestions = allProducts.filter(
              (p) => p.id !== product.id && collection && this.matchesCollection(p, collection),
            );

            // 2. Fallback to same category if not enough
    ...
    this.recentlyViewed$ = this.product$.pipe(
      ...
      switchMap(() => {
        ...
        return this.websiteService
          ...
          .pipe(
            map((products) =>
              products.filter((p) => ids.includes(p.id) && p.id !== currentId).slice(0, 3),
            ),
          );
      }),
    );
  ...
  isWholesaleCustomer(): boolean {
    const shop = this.shopContextService.getShopSync();
    if (!shop || !this.featureService.hasFeatureSync(shop, 'wholesale_system')) {
      return false;
    }
    const profile = this.authService.currentUserValue;
    return !!profile?.tags?.includes('wholesale');
  }

  hasColors(product: Product): boolean {
    const shop = this.shopContextService.getShopSync();
    if (!shop || !this.featureService.hasFeatureSync(shop, 'inv_color_inventory')) {
      return false;
    }
  ...
  hasSizes(product: Product): boolean {
    return product.variants.some((v) => !!v.size);
  }
  ...
  getAvailableSizes(product: Product): ProductVariant[] {
    // If color is selected, filter sizes by that color
  ...
  selectColor(color: string) {
    ...
    this.selectedSize = null; // Reset size on color change
  ...
  isColorAvailable(product: Product, color: string): boolean {
    ...
    // Usually color filters sizes. If a size is already selected, maybe we shouldn't block color clicking so user can switch colors and see different sizes available.
    // So we just check if the color has any stock overall.
    return variants.some((v) => (v.stock || 0) > 0);
  }

  isOutOfStock(product: Product): boolean {
    // Check if the currently selected variant is out of stock
    if (this.selectedVariant$.value) {
      return (this.selectedVariant$.value.stock || 0) <= 0;
    }
    // Otherwise check if all variants are out of stock
  ...
  addToCart(product: Product) {
    if (!this.authService.isAuthenticated()) {
      const slug = this.tenantService.getPathSlug();
      const loginUrl = slug ? `/shop/${slug}/auth/login` : '/login';
      this.router.navigate([loginUrl], { queryParams: { returnUrl: this.router.url } });
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (this.isWholesaleCustomer()) {
        const minQty = variant.wholesaleMinQty || product.variants[0]?.wholesaleMinQty || 1;
        if (this.quantity < minQty) {
          this.toastService.showError(`Wholesale orders require a minimum of ${minQty} units.`);
          this.isAdding = false;
          return;
        }
      }
  ...
  // Gallery Stack & Carousel Helpers
  ...
  // Wishlist and QuickAdd card actions
  ...
  async toggleFavorite(event: Event, productId: string) {
    ...
    if (user) {
      ...
      try {
        await this.authService.updateUserProfile(user.uid, { wishlist: currentWishlist });
      } catch (err) {
  ...
  // Review Logic and Ratings helpers
  ...
  submitReview() {
    ...
    this.websiteService.addReview(reviewData).subscribe({
      ...
      error: (err) => {
        console.error(err);
        const errorMsg = err?.error?.error?.message || err?.error?.message || 'Failed to submit review.';
  ...
  matchesCollection(product: Product, collectionName: string): boolean {
    ...
    if (product.tags && Array.isArray(product.tags)) {
      const hasTag = product.tags.some((t: string) => {
        ...
        return tagLower === colLower || tagLower.includes(colLower) || colLower.includes(tagLower);
      });
    ...
    if (colLower === 'party and events') {
      ...
      return keywords.some(
        (kw) => nameLower.includes(kw) || descLower.includes(kw) || subcatLower.includes(kw),
      );
    }
