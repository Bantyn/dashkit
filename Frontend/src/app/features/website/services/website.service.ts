...
export class WebsiteService {
  ...
  private getParams(params: HttpParams = new HttpParams()): HttpParams {
    // 1. Check for manual subdomain (Pro)
    const subdomain = this.tenantService.getSubdomainSlug();
    if (subdomain) {
      return params.set('subdomain', subdomain);
    }

    // 2. Check for path slug (Plus)
    const pathSlug = this.tenantService.getPathSlug();
    if (pathSlug) {
      return params.set('subdomain', pathSlug); // API expects 'subdomain' param for slug lookup currently
    }
  ...
  addReview(reviewData: any): Observable<any> {
    ...
    // Pass subdomain in query for shop resolution in middleware
    return this.http
