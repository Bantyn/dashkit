...
@Component({
  selector: 'app-festival-campaigns',
  standalone: true,
    ...
    <div class="h-full bg-[#f5f7fa] flex overflow-hidden">
      ...
      <div class="flex-1 flex flex-col min-w-0 border-r border-gray-200 bg-gray-50">
        <div class="px-6 py-5 border-b border-gray-200 bg-white flex justify-between items-center shrink-0">
          <h2 class="text-xl font-bold text-gray-900">Festival Campaigns</h2>
        ...
        <div class="p-4 bg-white shrink-0 border-b border-gray-100">
          <div class="relative">
            ...
            <input type="text" [(ngModel)]="searchQuery" placeholder="Search campaigns by title..." class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-transparent focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-lg text-sm transition-all" />
          </div>
        </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-gray-100">
                @if (loading) {
                  <tr><td colspan="4" class="px-6 py-8 text-center text-gray-500"><div class="flex flex-col items-center justify-center gap-4"><app-ui-loading size="md"></app-ui-loading><span>Loading...</span></div></td></tr>
                } @else if (error) {
                  <tr><td colspan="4" class="p-6"><div class="p-4 bg-red-50 text-red-600 rounded-lg text-center">{{ error }}</div></td></tr>
                } @else if (filteredItems.length === 0) {
                  <tr><td colspan="4" class="px-6 py-8 text-center text-gray-500">No campaigns found.</td></tr>
                } @else {
                  @for (item of filteredItems; track item.id) {
                    <tr (click)="selectItem(item)" class="cursor-pointer transition-colors hover:bg-gray-50 group" [class.bg-blue-50]="selectedItem?.id === item.id">
                      ...
                      <td class="px-6 py-4 text-sm text-gray-600">{{ (item.startDate | date) || '-' }}</td>
                      <td class="px-6 py-4 text-sm text-gray-600">{{ (item.endDate | date) || '-' }}</td>
                      <td class="px-6 py-4">
      ...
      @if (selectedItem) {
        <div class="w-[40%] min-w-[360px] max-w-[520px] bg-white border-l border-gray-200 flex flex-col h-full sticky top-0 overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 grid grid-cols-2 gap-4">
              ...
              @if (selectedItem.metadata?.discountType) {
                <div>
                  <span class="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Discount Type</span>
                  <span class="text-sm font-medium text-gray-900">{{ selectedItem.metadata.discountType }}</span>
                </div>
                <div>
                  <span class="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Discount Value</span>
                  <span class="text-sm font-medium text-gray-900">
                    {{ selectedItem.metadata.discountType === 'percentage' ? selectedItem.metadata.discountValue + '%' : '₹' + selectedItem.metadata.discountValue }}
                  </span>
                </div>
              }
            </div>
        ...
        <div class="hidden md:flex flex-col items-center justify-center w-[40%] min-w-[360px] max-w-[520px] bg-white border-l border-gray-200 text-center p-8 h-full sticky top-0">
          <div class="w-16 h-16 bg-primary-50 rounded-full flex items-center justify-center mb-4"><i class="bi bi-gift text-2xl text-primary-400"></i></div>
          <h3 class="text-gray-900 font-medium mb-1">No campaign selected</h3>
          <p class="text-gray-500 text-sm">Select a campaign from the list to view its details.</p>
        </div>
      }
    </div>
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
          <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 class="text-lg font-bold text-gray-900">{{ editingItem ? 'Edit Campaign' : 'New Campaign' }}</h3>
            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()">
              <div class="space-y-4">
                <div>
                  ...
                  <input type="text" formControlName="title" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm" placeholder="e.g. Diwali Sale" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                  <textarea formControlName="description" rows="3" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm" placeholder="Details..."></textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select formControlName="discountType" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm bg-white">
                  ...
                  <div>
                    ...
                    <input type="number" formControlName="discountValue" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm" placeholder="e.g. 10" />
                  </div>
                </div>
                <div class="flex items-center gap-2 mt-2">
                  ...
                  <label for="isActive" class="text-sm text-gray-700 cursor-pointer">Active Campaign</label>
                </div>
              </div>
...
export class FestivalCampaignsComponent implements OnInit {
  items: Promotion[] = [];
  loading = true;
  ...
  get filteredItems(): Promotion[] {
    const arr = this.items.filter(i => i.type === 'festival');
  ...
  loadItems() {
    ...
    this.service.getPromotions(this.shopId).subscribe({
      next: (response: { success: boolean; data: Promotion[] }) => {
        this.items = response.data.filter((p: Promotion) => p.type === 'festival');
        this.loading = false;
  ...
  onEdit(item: Promotion) {
    ...
    this.form.patchValue({
      ...
      discountValue: item.metadata?.discountValue || '',
