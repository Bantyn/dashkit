    ...
    <div class="h-full bg-[#f5f7fa] flex overflow-hidden relative">
      ...
      <div class="flex-1 flex flex-col min-w-0 border-r border-gray-200 bg-gray-50">
        <div class="px-6 py-5 border-b border-gray-200 bg-white flex justify-between items-center shrink-0">
          <h2 class="text-xl font-bold text-gray-900">SMS Campaigns</h2>
      ...
      @if (selectedItem) {
        <div class="w-[40%] min-w-[360px] max-w-[520px] bg-white border-l border-gray-200 flex flex-col h-full sticky top-0 overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            @if (selectedItem.metadata?.message) {
              <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <h5 class="text-xs font-bold text-blue-800 uppercase tracking-wider mb-2">Message Content</h5>
                <p class="text-sm text-blue-900 font-medium leading-relaxed">{{ selectedItem.metadata.message }}</p>
              </div>
            }
        ...
        <div class="hidden md:flex flex-col items-center justify-center w-[40%] min-w-[360px] max-w-[520px] bg-white border-l border-gray-200 text-center p-8 h-full sticky top-0">
          <div class="w-16 h-16 bg-primary-50 rounded-full flex items-center justify-center mb-4"><i class="bi bi-chat-text text-2xl text-primary-400"></i></div>
          <h3 class="text-gray-900 font-medium mb-1">No campaign selected</h3>
          <p class="text-gray-500 text-sm">Select an SMS campaign from the list to view its details.</p>
        </div>
      }
    </div>
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
          <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 class="text-lg font-bold text-gray-900">{{ editingItem ? 'Edit SMS Campaign' : 'New SMS Campaign' }}</h3>
            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()">
              <div class="space-y-4">
                <div>
                  ...
                  <input type="text" formControlName="title" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm" placeholder="e.g. Weekend Flash Sale" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">SMS Message</label>
                  <textarea formControlName="message" rows="3" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 text-sm" placeholder="Text message content..."></textarea>
                </div>
                <div>
...
export class SmsCampaignComponent implements OnInit {
  ...
  get filteredItems(): Promotion[] {
    const arr = this.items.filter(i => i.type === 'sms');
  ...
  loadItems() {
    ...
    this.service.getPromotions(this.shopId).subscribe({
      next: (response: { success: boolean; data: Promotion[] }) => {
        this.items = response.data.filter((p: Promotion) => p.type === 'sms');
        this.loading = false;
  ...
  async activateCampaign(item: Promotion) {
    const confirmed = await this.confirmationService.confirm({
      title: 'Send Campaign?',
      description: 'Are you sure you want to send this SMS campaign to your customers?',
      primaryButtonText: 'Send Now',
      secondaryButtonText: 'Cancel'
    });

    if (confirmed) {
      // Mocking audience for now. In real app, you would fetch actual customer numbers.
      ...
      this.http.post<any>(`${environment.apiUrl}/api/v1/admin/promotions/${item.id}/activate`, {
        ...
        next: (res) => {
          this.toastService.showSuccess("Campaign activated and queued!");
          this.loadItems();
        },
  ...
  onSubmit() {
    ...
    const req: any = { shopId: this.shopId, type: 'sms', metadata: { message: this.form.value.message }, ...this.form.value };
    delete req.message;
