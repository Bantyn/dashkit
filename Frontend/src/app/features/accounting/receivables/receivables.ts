...
import { AccountingService, ReceivableEntry } from '../../../core/services/accounting.service';
    ...
    <div class="h-[calc(100vh-64px)] bg-[#f5f7fa] flex flex-col overflow-hidden">
      ...
      <div class="px-6 py-5 border-b border-gray-200 bg-white flex flex-wrap justify-between items-center gap-4 shrink-0">
        <div>
          <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2">
            <i class="bi bi-clock-history text-amber-600"></i> Receivables
          </h2>
          <p class="text-xs text-gray-400 mt-0.5">Pending invoices — money owed to you by customers</p>
        </div>
      ...
      <div class="px-4 py-3 bg-white shrink-0 border-b border-gray-100 flex items-center gap-3">
        <div class="relative flex-1">
          ...
          <input type="text" [(ngModel)]="searchQuery" placeholder="Search customer or invoice..." class="w-full pl-9 pr-4 py-2 bg-gray-50 border border-transparent focus:bg-white focus:border-amber-400 focus:ring-2 focus:ring-amber-100 rounded-lg text-sm" />
        </div>
        <div class="text-sm font-bold text-amber-700 bg-amber-50 border border-amber-100 px-4 py-2 rounded-lg whitespace-nowrap">
          Total Outstanding: ₹{{ totalOutstanding | number }}
        </div>
      </div>
            ...
            <table class="w-full text-left border-collapse">
              <thead class="bg-gray-50 sticky top-0 z-10">
                <tr class="text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-200">
                  ...
                  <th class="px-5 py-3 text-right">Invoice Amt</th>
              ...
              <tbody class="divide-y divide-gray-50">
                @if (filteredEntries.length === 0) {
                  <tr><td colspan="7" class="py-12 text-center text-gray-400 text-sm">
                    ...
                    All receivables are clear!
                  </td></tr>
                } @else {
                  @for (entry of filteredEntries; track entry.invoiceId) {
                    <tr class="hover:bg-amber-50/30 transition-colors">
                      <td class="px-5 py-3 text-sm font-mono text-gray-700 font-semibold">{{ entry.invoiceNumber }}</td>
                      <td class="px-5 py-3">
                        <div class="text-sm font-medium text-gray-900">{{ entry.customerName }}</div>
                        <div class="text-xs text-gray-400">{{ entry.customerPhone }}</div>
                      </td>
                      <td class="px-5 py-3 text-xs text-gray-500">{{ entry.invoiceDate | date:'d MMM yy' }}</td>
                      ...
                      <td class="px-5 py-3 text-sm text-right font-bold text-red-600">₹{{ entry.outstanding | number }}</td>
                      <td class="px-5 py-3 text-center">
                        <span class="text-xs px-2 py-0.5 rounded-full font-semibold" [ngClass]="getAgingClass(entry.agingBucket)">
                          {{ entry.agingBucket }} days
                        </span>
                      </td>
                    </tr>
                  }
...
export class Receivables implements OnInit {
  ...
  get filteredEntries(): ReceivableEntry[] {
    let list = this.entries;
    if (this.bucketFilter) list = list.filter(e => e.agingBucket === this.bucketFilter);
    if (!this.searchQuery.trim()) return list;
    const q = this.searchQuery.toLowerCase();
    return list.filter(e => e.customerName.toLowerCase().includes(q) || e.invoiceNumber.toLowerCase().includes(q));
  }
  ...
  load() {
    ...
    this.loading = true;
    this.accountingService.getReceivables(this.shopId).subscribe({
      next: res => {
      ...
      error: () => { this.error = 'Failed to load receivables.'; this.loading = false; this.cdr.detectChanges(); },
    });
  }
