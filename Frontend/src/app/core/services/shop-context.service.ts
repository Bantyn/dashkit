...
export class ShopContextService {
  ...
  private isPublicSiteSubject = new BehaviorSubject<boolean>(false);
  ...
  constructor(
    ...
    // Subscribe to tenant changes to reload config
    this.tenantService.currentSlug$.subscribe((slug) => {
      this.isPublicSiteSubject.next(!!slug);
      if (slug) {
  ...
  setWorkspaceShopId(shopId: string | null) {
    this.workspaceShopIdSubject.next(shopId);
  }

  getWorkspaceShopId() {
    return this.workspaceShopIdSubject.value;
  }

  connectWorkspaceRoute(route: ActivatedRoute) {
    return combineLatest(
      route.pathFromRoot.map((level) =>
        level.paramMap.pipe(map((params) => params.get('shopId'))),
      ),
    ).pipe(
      map((shopIds) => shopIds.find((shopId) => !!shopId) || null),
      distinctUntilChanged(),
      tap((shopId) => this.setWorkspaceShopId(shopId)),
    );
  }

  loadPlatformTheme(): Observable<any> {
    return this.http.get<any>(`${environment.publicApiUrl}/themes`).pipe(
      tap((res) => {
        if (res.success && res.data?.shop) {
          this.applyTheme(res.data.shop);
        }
      }),
      catchError((err) => {
        console.warn('Failed to load platform theme settings', err);
        return of(null);
      })
    );
  }
  ...
  applyTheme(theme: any) {
    ...
    // New Tailwind Premium Aesthetic integration
    if (theme.primaryColor) {
      ...
      root.style.setProperty('--color-primary-600', hex); // Base
      ...
      // Lighter shades for backgrounds/borders/text hints
