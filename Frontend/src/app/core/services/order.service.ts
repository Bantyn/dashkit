...
export class OrderService {
  ...
  updatePaymentStatus(id: string, paymentStatus: string): Observable<ApiResponse<null>> {
    this.requestCache.clear();
    return this.http.put<ApiResponse<null>>(`${this.apiUrl}/${id}/payment-status`, { paymentStatus });
  }

  getOrdersByShop(shopId: string): Observable<ApiResponse<Order[]>> {
    return this.requestCache.getOrSet(
      `orders:shop:${shopId}`,
      () => this.http.get<ApiResponse<Order[]>>(`${this.apiUrl}/shop/${shopId}`),
      this.ttlMs,
    ) as Observable<ApiResponse<Order[]>>;
  }

  verifyPayment(id: string, paymentDetails: { razorpayPaymentId: string; razorpayOrderId: string; razorpaySignature: string }): Observable<ApiResponse<any>> {
    this.requestCache.clear();
