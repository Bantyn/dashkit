...
import { of } from 'rxjs';
import {
  getRouteParamFromSnapshot,
  getWorkspaceBlockedPath,
} from '../utils/workspace-route.utils';
...
export const enforcementGuard: CanActivateFn = (route, state) => {
  ...
  const shopId = getRouteParamFromSnapshot(route, 'shopId') || user?.shopId;
  if (!user) {
  ...
  if (requiredPermission && !permissionService.hasPermission(requiredPermission)) {
    router.navigate(['/', shopId, 'access-denied'], {
      queryParams: {
        ...
        blockedPath: getWorkspaceBlockedPath(state.url),
        requiredPermission,
      },
    });
    return false;
